...
import { canCustomerCancel, statusNote } from "@/lib/delivery-tracking";
...
export async function POST(_request: Request, context: RouteContext) {
  ...
  await notifyAdminAboutCustomerAction(id, "CANCELLED");
...
async function notifyAdminAboutCustomerAction(orderId: string, status: string) {
  ...
  if (failed.length > 0) {
    console.error("Admin order cancellation notification failed", {
      orderId,
